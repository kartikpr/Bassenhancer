...
#include "../Blockdiagram/RAFX2 WinBuild/kpFx.h"
...
PluginCore::PluginCore()
{
	...
	AudioFilterParameters LPparams = lowPass.getParameters();
	//LPparams.fc = 70;
	LPparams.Q = 8;
	LPparams.algorithm = filterAlgorithm::kLWRLPF2;
	lowPass.setParameters(LPparams);

	AudioFilterParameters HPparams = highPass.getParameters();
	//HPparams.fc = 70;
	HPparams.Q = 8;
	HPparams.algorithm = filterAlgorithm::kLWRHPF2;
	highPass.setParameters(HPparams);

	AudioFilterParameters HShelf = highSelfHighSignal.getParameters();
	HShelf.fc = 6000;
	HShelf.algorithm = filterAlgorithm::kHiShelf;
	highSelfHighSignal.setParameters(HShelf);

	AudioFilterParameters band = bandpass.getParameters();
	band.fc = 800;
	band.Q = 8;
	band.algorithm = filterAlgorithm::kBPF2;
	bandpass.setParameters(band);
...
void PluginCore::updateParameters() {
	...
	AudioFilterParameters paramsLP = lowPass.getParameters();
	paramsLP.fc = m_fMyVariable;
	lowPass.setParameters(paramsLP);

	AudioFilterParameters paramsHP = highPass.getParameters();
	paramsHP.fc = m_fMyVariable;
	highPass.setParameters(paramsHP);

	AudioFilterParameters HShelf = highSelfHighSignal.getParameters();
	HShelf.boostCut_dB = highShelfDb;
	highSelfHighSignal.setParameters(HShelf);
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	float& leftIn = processFrameInfo.audioInputFrame[0];
	float& leftOut = processFrameInfo.audioOutputFrame[0];
	float& rightIn = processFrameInfo.audioInputFrame[1];
	float& rightOut = processFrameInfo.audioOutputFrame[1];

	float leftOutLP = lowPass.processAudioSample(leftIn);
	float rightOutLP = lowPass.processAudioSample(rightIn);

	float leftOutHP = highPass.processAudioSample(leftIn);
	float rightOutHP = highPass.processAudioSample(rightIn);

	//float shelfedHighL = highSelfHighSignal.processAudioSample(leftOutHP);
	//float shelfedHighR = highSelfHighSignal.processAudioSample(rightOutHP);
    ...
    // --- Stereo-In/Stereo-Out
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		float distortedL = FX_Class::FX<float>::distortion(leftOutLP, preGain, drive);
		float distortedR = FX_Class::FX<float>::distortion(rightOutLP, preGain, drive);

		float distortedLBP = bandpass.processAudioSample(distortedL);
		float distortedRBP = bandpass.processAudioSample(distortedR);

		float leftOutWOHP = wet*(distortedLBP) + dry*(leftIn);
		float rightOutWOHP = wet*(distortedRBP)+ dry*(rightIn);

		leftOut = highSelfHighSignal.processAudioSample(leftOutWOHP);
		rightOut = highSelfHighSignal.processAudioSample(rightOutWOHP);
...
bool PluginCore::initPluginParameters()
{
	...
	// --- continuous control: Clean Gain
	piParam = new PluginParameter(controlID::preGain, "Clean Gain", "Units", controlVariableType::kFloat, 0.000000, 10.000000, 1.000000, taper::kLinearTaper);
	...
	piParam = new PluginParameter(controlID::drive, "Drive", "Units", controlVariableType::kFloat, 1.000000, 10.000000, 1.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&drive, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: Cross-Over
	piParam = new PluginParameter(controlID::m_fMyVariable, "Cross-Over", "Hz", controlVariableType::kFloat, 20.000000, 150.000000, 70.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&m_fMyVariable, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: POST High Shelf Gain/Cut
	piParam = new PluginParameter(controlID::highShelfDb, "POST High Shelf Gain/Cut", "Units", controlVariableType::kFloat, -5.000000, 5.000000, 0.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&highShelfDb, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: Wet
	piParam = new PluginParameter(controlID::wet, "Wet", "Units", controlVariableType::kFloat, 0.000000, 16.000000, 0.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&wet, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: Dry
	piParam = new PluginParameter(controlID::dry, "Dry", "Units", controlVariableType::kFloat, 0.000000, 1.000000, 1.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(20.00);
	piParam->setBoundVariable(&dry, boundVariableType::kFloat);
	addPluginParameter(piParam);
	...
	// --- controlID::m_fMyVariable
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::m_fMyVariable, auxAttribute);

	// --- controlID::highShelfDb
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::highShelfDb, auxAttribute);

	// --- controlID::wet
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483648);
	setParamAuxAttribute(controlID::wet, auxAttribute);
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::preGain, 1.000000);
	setPresetParameter(preset->presetParameters, controlID::drive, 1.000000);
	setPresetParameter(preset->presetParameters, controlID::m_fMyVariable, 70.000000);
	setPresetParameter(preset->presetParameters, controlID::highShelfDb, 0.000000);
	setPresetParameter(preset->presetParameters, controlID::wet, 0.000000);
